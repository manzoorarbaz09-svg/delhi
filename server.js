...
function computeStats(values) {
  ...
  // Mode (approximate — most frequent bin)
  const binSize = 50;
  const bins = {};
  values.forEach(v => {
...
function predict(type, month, day, week, targetYear, method) {
  ...
  if (type === 'year') {
    ...
    const globalStdV = globalStats.std;
    ciLower = Math.max(0, Math.round(predicted - 1.96 * globalStdV));
    ciUpper = Math.round(predicted + 1.96 * globalStdV);
    confidence = 72;
...
// GET /api/stats — all precomputed stats
...
// POST /api/predict — statistical prediction
...
// GET /api/daily/:year/:month — daily data for a specific year+month
...
// POST /api/ai-analyze — Claude AI analysis via Anthropic API
app.post('/api/ai-analyze', async (req, res) => {
  const { question, context } = req.body;

  // Build a rich context string from our real data
  const dataContext = `
...
// GET /api/forecast/:year — full year month-by-month forecast
